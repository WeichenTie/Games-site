...
const MainLobby = () => {
    const clientMethods = async (useLobbyHook) => {
        await useLobbyHook.connection.on('JoinLobbyWithId', (url) => {
            useLobbyHook.navigate.current(url);
        });
    }

    const lobbyHook = useLobby('http://127.0.0.1:5000/main-lobby', 'HOME', clientMethods);
